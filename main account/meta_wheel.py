"""
META Wheel Strategy — Paper Trading
...
BUYING_POWER_LIMIT = 500_000   # max total cash committed across all put positions
STATE_FILE      = "meta_wheel_state.json"
...
def get_committed_cash():
    """Total cash committed by all open short put positions in the account."""
    try:
        positions = api_get("/positions")
        total = 0.0
        for p in positions:
            sym = p.get("symbol", "")
            if p.get("asset_class") == "us_option" and "P" in sym and float(p.get("qty", 0)) < 0:
...
def do_sell_put(state):
    ...
    if committed + needed > BUYING_POWER_LIMIT:
        log(f"SKIP PUT -- buying power limit: committed ${committed:,.0f} + new ${needed:,.0f} > ${BUYING_POWER_LIMIT:,.0f} cap")
        return state
    ...
    if position is None:
        ...
        if stage == "PUT":
            ...
            if stock:
                ...
                save_state(state)
                log(f"*** ASSIGNED ***  Bought {shares} META @ ${avg_entry:.2f}")
...
def main():
    print("=" * 60)
    print(f"  META WHEEL STRATEGY  —  Paper Trading")
